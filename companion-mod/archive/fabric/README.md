...
These are fully built, compile-verified Fabric projects for **intermediate 1.21.x
render-state eras** that the launcher no longer ships:
...
the launcher actively supports only three eras — `1.21-1.21.1` (feature-renderer),
`1.21.11` (= 26.x source), and `26.1-26.2`. These three intermediate eras were
moved out of `companion-mod/fabric/` so they're not built by CI, not advertised by
the launcher, and not listed in the published manifest — but the source is kept,
...
3. Re-add its versions to the launcher gate: `version_supported()` in
