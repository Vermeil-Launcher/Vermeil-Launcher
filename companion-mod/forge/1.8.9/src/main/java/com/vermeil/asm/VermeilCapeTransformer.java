 ...
 * our texture. When it returns null, the original vanilla logic runs unchanged,
 * so every other player keeps their real Mojang cape.
 *
 * <p>Frame handling: we read with {@code EXPAND_FRAMES} and write with
 * {@code COMPUTE_MAXS}, supplying the single stack-map frame our branch target
 * needs by hand. This avoids recomputing frames for the whole class (which would
...
public class VermeilCapeTransformer implements IClassTransformer {
	...
	public byte[] transform(final String name, final String transformedName, final byte[] basicClass) {
		...
		for (MethodNode method : node.methods) {
			...
			head.add(new JumpInsnNode(Opcodes.IFNULL, afterReturn));
			head.add(new InsnNode(Opcodes.ARETURN));
			head.add(afterReturn);
